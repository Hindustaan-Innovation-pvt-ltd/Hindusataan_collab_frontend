...
function TableNode({ el, selected, editingId, zoom, onBlur, onDblClick, onUpdate }: TableNodeProps) {
  ...
  return (
    ...
    >
      <div
        className="absolute inset-0 bg-card border-2 border-border rounded overflow-hidden flex flex-col shadow-sm"
        style={{ boxShadow: selected ? "0 0 0 2px #3742FA" : undefined }}
      >
        {Array.from({ length: rows }).map((_, r) => (
          <div key={r} className="flex flex-1 w-full border-b border-border last:border-b-0">
            {Array.from({ length: cols }).map((_, c) => {
              ...
              return (
                <div
                  key={c}
                  className="flex-1 border-r border-border last:border-r-0 relative flex items-center justify-center p-1.5 hover:bg-background/80 transition-colors"
                  onPointerDown={(e) => {

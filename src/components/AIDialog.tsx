        ...
        () => {
          setMessages(p => {
            ...
            if (msg) {
              ...
              // We parse out JSON action blocks more robustly (handles single, triple, or no backticks)
              const regex = /(?:```(?:json)?|`)?\s*(\{\s*"action"\s*:\s*"[^"]+"[\s\S]*?\})\s*(?:```|`)?/g;
              let match;
              ...
              if (hasAction) {
                msg.content = msg.content.replace(/(?:```(?:json)?|`)?\s*\{\s*"action"\s*:\s*"[^"]+"[\s\S]*?\}\s*(?:```|`)?/g, "").trim();
